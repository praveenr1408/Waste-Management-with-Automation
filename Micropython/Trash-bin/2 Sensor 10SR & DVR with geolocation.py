...
# Functions to fetch public IP and location
...
while True:
    try:
        ...
        if (previous_distance_aj is None or abs(previous_distance_aj - distance_aj) > distance_threshold or
            ...
            # Update previous distance measurements
            previous_distance_aj = distance_aj
            previous_distance_hc = distance_hc

        # Send periodic updates every status_update_interval seconds
        if time.time() - last_status_update_time > status_update_interval:
            ...
            # Record last status update time
